...
Q3_OP_SLOTS = {8: 8, 9: 6, 10: 0, 11: 4, 12: 6, 13: 0, 14: 4, 15: 8} #q3
...
def office_hour(start_t, oh):
    ...
    while oh > 1e-12:
        d = int(t/24); oh_end = d*24+16; oh_left = oh_end-t
        if oh_left >= oh: return t+oh
        oh -= oh_left; t = start_working(oh_end)
    return t
...
def slot_begin(slot_table, earliest_day, latest_day):
    for d in range(earliest_day, latest_day+1):
        if d%7 >= 5: continue
        for hour in range(8, 16):
            for s in range(slots_for_hour(hour)):
...
def patient_new(ptype, t):
    global patient_count; patient_count += 1
...
class Scanning:
    def __init__(self):
        self.t = 0; self.future_list = []
        self.emergency_queue = []; self.queue_normal = []; self.waiting_list = []
        self.warmup_done = False
        self.sc_busy = [False, False]; self.sc_available = [True, False]
        self.sc_busy_since = [None, None]; self.sc_patient = [None, None]
        self.slot_table = {}
        self.batch_busy_oh = [0.,0.]; self.batch_busy_noh = [0.,0.]
        self.batch_oh_dur = 0.; self.batch_noh_dur = 0.; self.period_t = 0.
        self.b_sc1_oh=[]; self.b_sc2_oh=[]; self.b_sc1_noh=[]
        self.b_op_acc=[]; self.b_em_wait=[]; self.b_op_wait=[]
        self.b_ovfl=[]; self.b_inp_rate=[]
        self.cur_op_acc=[]; self.cur_em_wait=[]; self.cur_op_wait=[]
        self.cur_ovfl=[]; self.cur_inp_rate=[]
...
def busy(sim, idx, from_t, to_t):
    t = from_t
    while t < to_t-1e-12:
        nxt = min(next_loop(t), to_t); dt = nxt-t
        if is_office_hours(t): sim.batch_busy_oh[idx] += dt
        else:                  sim.batch_busy_noh[idx] += dt
        t = nxt
...
'''
def freesc(sim):
    if sim.sc_available[sc2] and not sim.sc_busy[sc2]: return sc2
    if sim.sc_available[sc1] and not sim.sc_busy[sc1]: return sc1
    return None
'''
def freesc(sim):
...
def start_scan(sim, sc_idx, patient):
    ...
    sim.sc_busy[sc_idx] = True; sim.sc_patient[sc_idx] = patient
    sim.sc_busy_since[sc_idx] = sim.t; patient['svc_start'] = sim.t
...
def start_service(sim, patient, wait_hours):
    ...
    if patient['type'] == pt_e: sim.cur_em_wait.append(wait_hours)
    elif patient['type'] == pt_o: sim.cur_op_wait.append(wait_hours)
...
def next_scan(sim, sc_idx):
    if sim.emergency_queue:
        ...
        if inps and ((h >= 9.5 and h<14.25) or (not ops)):
            nxt = inps[0]; sim.queue_normal.remove(nxt)
        elif ops and (h >=12.75 and h < 13.25 and len(ops)<=3):
            nxt = ops[0]; sim.queue_normal.remove(nxt)
    ...
    start_service(sim, nxt, wait); start_scan(sim, sc_idx, nxt)

def update_queue(sim, patient):
    if len(sim.emergency_queue)+len(sim.queue_normal) >= chair_limit:
        patient['outside_room'] = True
    if patient['type'] == pt_e: sim.emergency_queue.append(patient)
    else:                        sim.queue_normal.append(patient)


def eme_arrive(sim):
    t = sim.t
    schedule(sim.future_list, t+random.expovariate(lambda_e), "eme_arrival")
    p = patient_new(pt_e, t); sc = freesc(sim)
    if sc is not None: start_service(sim,p,0.); start_scan(sim,sc,p)
    else: update_queue(sim,p)

def inp_arrive(sim):
    t = sim.t
    schedule(sim.future_list, next_inp(t), "inp_arrival")
    p = patient_new(pt_i, t); p['oh_request'] = is_office_hours(t)
    p['oh_req_day'] = day_number(t); sc = freesc(sim)
    if sc is not None: start_service(sim,p,0.); start_scan(sim,sc,p)
    else: update_queue(sim,p)
...
def op_called(sim):
    ...
    earliest = day_number(t)+1; this_fri = day_number(t)+(4-week_day(t))
    if earliest > this_fri: sim.waiting_list.append(p); return
    mon = day_number(t)-week_day(t)
    slots_week(sim.slot_table, mon)
    found = slot_begin(sim.slot_table, earliest, this_fri)
    if found:
        d,hour,s = found; sim.slot_table[(d,hour,s)] = p['pid']
        p['appt_t'] = slot_to_time(d,hour,s); p['appt_day'] = d
        if random.random() <= p_show:
            schedule(sim.future_list, p['appt_t'], "op_arrival", {'p': p})
    else: sim.waiting_list.append(p)

def op_arrive(sim, patient):
    t = sim.t; patient['arrival_t'] = t
    if sim.warmup_done:
        sim.cur_op_acc.append(patient['appt_day']-patient['request_day'])
    sc = freesc(sim)
    if sc is not None: start_service(sim,patient,0.); start_scan(sim,sc,patient)
    else: update_queue(sim,patient)

def scan_finish(sim, scanner):
    p = sim.sc_patient[scanner]; p['svc_end'] = sim.t
    if sim.warmup_done and sim.sc_busy_since[scanner] is not None:
        busy(sim, scanner, sim.sc_busy_since[scanner], sim.t)
    sim.sc_busy[scanner]=False; sim.sc_patient[scanner]=None
...
def friday_batch(sim):
    ...
    #print(day_number(sim.t),len(sim.waiting_list),sim.sc2_half_days)
    sim.sc2_half_days = scanner2_policy(len(sim.waiting_list))

    mon_next = day_number(sim.t)+3
    slots_week(sim.slot_table, mon_next)
    while sim.waiting_list:
        p = sim.waiting_list[0]
        found = slot_begin(sim.slot_table, mon_next, mon_next+4)
        if found:
            sim.waiting_list.pop(0); d,hour,s = found
...
def warmup_event(sim):
    sim.warmup_done=True; sim.period_t=sim.t
    for i in range(2):
...
def batch_event(sim):
    ...
    def bm(lst): return mean(lst) if lst else float('nan')
    ...
    sim.batch_busy_oh=[0.,0.]; sim.batch_busy_noh=[0.,0.]
    sim.batch_oh_dur=0.; sim.batch_noh_dur=0.
    sim.batch_sc2_open_dur = 0.
    sim.cur_op_acc=[]; sim.cur_em_wait=[]; sim.cur_op_wait=[]
    sim.cur_ovfl=[]; sim.cur_inp_rate=[]
    if sim.t+batch_hours < total_hours:
        schedule(sim.future_list, sim.t+batch_hours, "batch")


def simulation():
    global patient_count; patient_count=0
    sim = Scanning(); fl = sim.future_list
    ...
    while fl:
        ...
        sim.t = ev_time
        if   ev_type=="eme_arrival": eme_arrive(sim)
        elif ev_type=="inp_arrival": inp_arrive(sim)
        elif ev_type=="op_call":     op_called(sim)
        elif ev_type=="op_arrival":  op_arrive(sim, ev_data['p'])
        elif ev_type=="scan":        scan_finish(sim, ev_data['sc'])
        elif ev_type=="open_sc2":    sc2_open(sim)
        elif ev_type=="close_sc2":   sc2_close(sim)
        elif ev_type=="friday":      friday_batch(sim)
        elif ev_type=="warmup":      warmup_event(sim)
        elif ev_type=="batch":       batch_event(sim)
        elif ev_type=="endsim":      break
    return sim
...
if __name__ == "__main__":
    def ci_hw(lst):
        ...
        m=mean(lst); s=math.sqrt(sum((x-m)**2 for x in lst)/(n-1))
        return 2.04523*s/math.sqrt(n)

    def print_res(label, lst, scale=1., unit=""):
        m=mean(lst); hw=ci_hw(lst)
        print(f"  {label}: {m*scale:.4f}{unit}  95%CI=[{(m-hw)*scale:.4f},{(m+hw)*scale:.4f}]")


    sim = simulation()
    print_res("SC1 oh utilization",      sim.b_sc1_oh)
    print_res("SC2 oh utilization",      sim.b_sc2_oh)
    print_res("SC1 noh utilization",  sim.b_sc1_noh)
    print_res("Outpatient access time",     sim.b_op_acc,    unit=" days")
    print_res("Emergency waiting time",        sim.b_em_wait,   scale=60, unit=" min")
    print_res("Outpatient waiting time",       sim.b_op_wait,   scale=60, unit=" min")
    print_res("Overflow fraction",          sim.b_ovfl)
    print_res("Inpatient missing",    sim.b_inp_rate)